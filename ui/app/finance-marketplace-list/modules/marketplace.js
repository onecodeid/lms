...
// 2 => DONE
// 3 => ERROR
import * as api from "./api_marketplace.js"
...
export default {
    ...
    actions: {
        ...
        async save_mp(context) {
            context.commit("update_search_status", 1)

            try {
                let prm = {}, __s = context.state
                prm.token = one_token()
                prm.so_ads_number = __s.so_ads_number
                prm.so_mp_number = __s.so_mp_number
                prm.so_mp_cost = __s.so_mp_cost
                prm.so_id = __s.selected_order.L_SoID

                let resp = await api.save_mp(prm)
                ...
                if (resp.status != "OK") {
                    ...
                    context.commit("set_object", ["dialog_item", false])
                    return resp.data
                }
            } catch (e) {
                context.commit("update_search_status", 3)
