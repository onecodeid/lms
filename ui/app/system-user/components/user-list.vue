            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td class="text-xs-center pa-0 pr-1 pl-1" @click="select(props.item)">
                        <v-btn color="orange" class="btn-icon ma-0" small @click="username(props.item)" :disabled="props.item.user_id == me.user_id || props.item.user_name == 'admin'" dark>
...
module.exports = {
    components : {
        ...
        'user-name-change' : httpVueLoader('./user-name-change.vue'),
    ...
    methods : {
        ...
        edit (x) {
            ...
            this.$store.commit('user_new/set_fees', x.fees)
        ...
        add (x) {
            ...
            this.$store.commit('user_new/set_fees', this.$store.state.user_new.fee_template)
            // this.$store.commit('user/set_common', ['user_name', x.user_name])
        },
        ...
        username (x) {
            this.select(x)
            this.$store.commit('user_new/set_common', ['username_error', false])
            this.$store.commit('user_new/set_common', ['dialog_username', true])
            this.$store.commit('user_new/set_common', ['error_check', false])

            this.$store.commit('user_new/set_common', ['old_user_name', x.user_name])
            this.$store.commit('user_new/set_common', ['user_name', ''])
            this.$store.commit('user_new/set_common', ['user_pass', ''])
