    ...
    >
        <v-card>
            ...
            <v-card-actions>
                ...
                <v-btn color="success" @click="generate('xls')" class="btn-icon" :disabled="!selected_admin" title="Download EXCEL">Excel</v-btn>
                <v-btn color="orange" :dark="!!selected_admin" @click="generate()"
                    :disabled="!selected_admin" class="btn-icon" title="Cetak PDF"><v-icon class="mr-1">print</v-icon> PDF</v-btn>
            </v-card-actions>
        </v-card>
...
module.exports = {
    ...
    computed : {
        dialog : {
            get () { return this.$store.state.report_param.dialog['sales-010'] },
            set (v) { this.$store.commit('report_param/set_dialog', ['sales-010', v]) }
        },
        ...
        params () {
            return ['sdate='+this.sdate, 'edate='+this.edate, 'uid='+this.selected_admin.user_id, 'levelid='+this.selected_level.M_CustomerLevelID].join('&')
        },
