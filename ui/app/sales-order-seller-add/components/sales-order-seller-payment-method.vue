                    ...
                    <v-radio-group v-model="selected_payment_id" class="mt-1 pt-0">
                        <v-card v-for="(p, i) in payments" v-bind:key="i" class="mb-2">
                            ...
                            <v-card-text class="pa-0">
                                <v-layout row wrap>
                                    <v-flex xs3 class="blue lighten-5 pa-2">
                                        <v-radio :label="p.M_PaymentTypeName" :value="p.M_PaymentTypeID" color="orange">
                                            <template slot="label">
                                                <h3 class="subheading">{{ p.M_PaymentTypeName }}</h3>
                                                
                                                <div class="caption red--text" v-show="p.M_PaymentTypeCode == 'IPAYMU.CS'">( Pilih salah satu )</div>
                                            </template>
                                        </v-radio>
                                        
                                    </v-flex>
                                    <v-flex xs9 pa-2>
                                        <v-list two-line v-show="p.M_PaymentTypeCode == 'TRANSFER'" class="layout row wrap pt-0">
                                            ...
                                            <template v-for="(item, index) in accounts">
                                                ...
                                                >
                                                    <v-list-tile-avatar class="mr-3">
                                                        <v-img :src="'../'+item.bank_logo" width="72" height="36"></v-img>
                                                    </v-list-tile-avatar>

                                                    <v-list-tile-content>
                                                        <v-list-tile-title v-html="item.bank_name" class="body-2"></v-list-tile-title>
                                                        <v-list-tile-sub-title v-html="item.account_number+' a/n '+item.account_name_only"></v-list-tile-sub-title>
                                                    </v-list-tile-content>
                                                </v-list-tile>
                                            </template>
                                        </v-list>

                                        <v-list two-line v-show="p.M_PaymentTypeCode == 'IPAYMU.CS'" class="layout row wrap pt-0">
                                            ...
                                            <template v-for="(item, index) in channels">
                                                ...
                                                v-bind:class="is_selected_channel_color(item)"
                                                >
                                                    <v-list-tile-avatar class="mr-3">
                                                        <v-img :src="'../assets/img/logo-'+item.value+'.png'" width="72" height="36"></v-img>
                                                    </v-list-tile-avatar>

                                                    <v-list-tile-content>
                                                        <v-list-tile-title v-html="item.text" class="body-2"></v-list-tile-title>
                                                        <!-- <v-list-tile-sub-title v-html="item.account_number+' a/n '+item.account_name_only"></v-list-tile-sub-title> -->
                                                    </v-list-tile-content>
                                                </v-list-tile>
                                        ...
                                        <v-img src="../assets/img/qris.png" v-show="p.M_PaymentTypeCode == 'IPAYMU.QRIS'" width="80%" height="80%" contain></v-img>
                                    </v-flex>
                                </v-layout>
                    ...
                    <v-card class="mt-1">
                        ...
                        <v-card-text>
                            ...
                            <v-layout row wrap pt-2>
                                ...
                                <v-flex xs6 class="text-xs-right">Rp {{ one_money(0) }}</v-flex>
                            </v-layout>
                            ...
                            <v-layout row wrap>
                                ...
                                <v-flex xs6 v-show="coupon_set">
                                    Kupon : <b>{{coupon_code}}</b> <a href="javascript:;" @click="coupon_clear"><v-icon small>clear</v-icon></a>
                                </v-flex>
    ...
    computed : {
        ...
        cod_cost () {
            ...
            if(this.selected_expedition.M_ExpeditionIsCOD=="Y") {
                if (this.selected_expedition.M_ExpeditionCODRateItemsOnly=="N")
                    return Math.round(this.selected_expedition.M_ExpeditionCODRate * this.grand_total_price / 100)
                else
                    return Math.round(this.selected_expedition.M_ExpeditionCODRate * (this.total_price - parseFloat(this.coupon_amounts)) / 100)
            }
