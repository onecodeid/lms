    ...
    >
        <v-card>
            <v-card-title primary-title class="blue white--text pt-2 pb-2">
                <h3 class="headline">Cetak Kartu Stok</h3>
            </v-card-title>
            <v-card-text>
                <v-layout row wrap>
                    <v-flex xs12>
...
module.exports = {
    ...
    computed : {
        ...
        item_name () {
            return this.$store.state.item.selected_item.M_ItemName
        }
    },
    ...
    methods : {
        ...
        print_me () {
            let r = `${this.$store.state.item.URL}report/one_wh_005?item_id=\
                        ${this.$store.state.item.selected_item.M_ItemID}&sdate=\
                        ${this.sdate}&edate=${this.edate}`
