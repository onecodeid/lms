         ...
         <v-app id="oneApp"  v-if="one_token">
            ...
            <common-navbar></common-navbar>
               <v-container pt-1 pl-1 pr-1 fluid>
                  <v-layout row :class="{'px-4':$vuetify.breakpoint.mdOnly}">
                     <v-flex xs12 md12 lg8 xl6 offset-lg2 offset-xl3>
                        <crm-info-header></crm-info-header>
...
mounted: function() {
    ...
    <?php
    if (isset($_GET['id'])) {
        echo 'this.$store.commit("infoPreview/set_object", ["info_id", '.$_GET['id'].']);this.$store.dispatch("infoPreview/getOne");';
    }
    ?>
},
computed : {
...
methods : {
   ...
   components: {
        ...
        "crm-info-preview" : httpVueLoader("../components/crm-info-preview.vue<?=$ts;?>"),
        "crm-info-header" : httpVueLoader("../components/crm-info-header.vue<?=$ts;?>")
      }
    })
    </script>
    <style>
