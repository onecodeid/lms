        ...
        <v-card-text class="pt-2">
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td class="text-xs-left pa-2" @click="select(props.item)">{{ props.item.info_excerpt != '' ? props.item.info_excerpt : props.item.info_content.substr(0, 150) }}</td>   
                    ...
                    <td class="text-xs-center pa-0" @click="select(props.item)">
                        ...
                        <v-btn color="primary" class="btn-icon ma-0" small @click="get(props.item)"><v-icon>visibility</v-icon></v-btn>
...
module.exports = {
    ...
    methods : {
        ...
        add () {
            ...
            this.$store.commit('info_new/set_object', ['selected_users', []])
            this.$store.commit('info_new/set_object', ['who_read', []])
        },
        ...
        edit (x) {
            ...
            this.$store.commit('info_new/set_common', ['info_img', sc.info_img])
            ...
            this.$store.commit('info_new/set_common', ['info_img_url', sc.img_url])
            this.$store.commit('info_new/set_content', sc.info_content)
            this.$store.commit('info_new/set_object', ['who_read', sc.who_read])
            this.$store.commit('info_new/set_object', ['customer_read_cnt', sc.customer_read_cnt])

            let rcps = []
            for (rc of sc.recipients) {
                for (usr of this.$store.state.info_new.users) {
                    if (!usr.header && usr.id == rc.id && usr.group == rc.group) {
                        rcps.push(usr)
                    }
                }
            }
            this.$store.commit('info_new/set_object', ['selected_users', rcps])

            rcps = []
            for (rc of sc.levels) {
                for (usr of this.$store.state.info_new.customer_levels) {
                    if (usr.level_id == rc.level_id) {
                        rcps.push(usr)
                    }
                }
            }
            this.$store.commit('info_new/set_object', ['selected_customer_levels', rcps])

            this.$store.dispatch('info_new/getBase64Image').then((x) => {
        ...
        get (x) {
            ...
            this.$store.dispatch('infoPreview/getOne').then((x) => {
