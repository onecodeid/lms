<template>
    <v-card>
        <!-- <v-card-title primary-title class="cyan white--text pt-3">
            <h3><a href="../feed" class="white--text">DAFTAR INFORMASI</a> » <span class="">INFORMASI</span></h3>
        </v-card-title> -->
        <v-card-text>
            <v-card>
                <v-img
                ...
                <v-card-title primary-title>
                    <div>
                        <h3 class="headline mb-0">{{ info_title }}</h3>
                        <span class="mt-0 mb-3 caption d-block"><i>{{ info_date }}</i></span>
                        <div v-html="info_content"></div>
                    </div>
                </v-card-title>

                <!-- <v-card-actions><v-spacer></v-spacer>
                <v-btn flat color="red">Saya Sudah Baca</v-btn>
                <v-btn flat color="orange" v-show="m.notif_msg_full.length>250">Baca Detail</v-btn>
                </v-card-actions> -->
            </v-card>
        </v-card-text>
...
module.exports = {
    ...
    computed : {
        ...
        info_date () {
            return this.$store.state.infoPreview.info_date
        },
