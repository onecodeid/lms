    ...
    actions: {
        async search(context) {
            ...
            context.dispatch("system/postme", {
                url: "notice/message/search",
                prm: prm,
                callback: function(d) {
                    context.commit("set_object", ['infos', d.records])
                    context.commit("set_object", ['total_info', d.total])
                    context.commit("set_object", ['total_info_page', d.total_page])
                }
            }, { root: true })

        },

        async searchFeed(context) {
            let prm = {
                token: one_token(),
                search: context.state.search,
                page: context.state.current_page,
                limit: context.state.feed_limit
            }

            context.dispatch("system/postme", {
                url: "notice/message/search_feed",
                prm: prm,
                callback: function(d) {
                    context.commit("set_object", ['infos', d.records])
                    context.commit("set_object", ['total_info', d.total])
        ...
        async del(context) {
            let prm = {
                token: one_token(),
                message_id: context.state.selected_info.info_id
            }

            context.dispatch("system/postme", {
                url: "notice/message/delete",
                prm: prm,
                callback: function(d) {
                    context.dispatch("search")
                }
