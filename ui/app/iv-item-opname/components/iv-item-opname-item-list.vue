...
module.exports = {
    ...
    computed : {
        dialog : {
            get () { return this.$store.state.opname_new.dialog_item },
            set (v) { this.$store.commit('opname_new/set_common', ['dialog_item', v]) }
        },

        items () { 
            return this.$store.state.opname_new.items_av
        }
    },

    methods : {
        select (x) {
            this.$store.commit('opname_new/set_selected_item_av', x)
        },

        pick (x) {
            this.select(x)
            let items = this.$store.state.opname_new.items
            ...
            this.$store.commit('opname_new/set_items', items)
            // this.dialog = false
        },

        is_selected (item) {
            let itm = this.$store.state.opname_new.items
            for (let i of itm) {
