...
class User extends MY_Controller
{
    ...
    */
    ...
    {
        $r = $this->s_user->username_change($this->sys_input,
                $this->sys_user['user_id'], 
                $this->sys_input['user_pass']);
    ...
    function search_dd_with_group()
    {
        ...
        foreach ($r['records'] as $k => $v)
            // $dd['Grup'][] = ['id' => $v['group_id'], 'text' => 'Grup ' . $v['group_name']];
            $dd[] = ['id' => $v['group_id'], 'text' => 'Grup ' . $v['group_name'], 'group' => true];

        // user
        $r = $this->s_user->search(['search'=>'%', 'gid'=>0]);
        $dd[] = ['header' => 'User'];
        foreach ($r['records'] as $k => $v)
            // $dd['User'][] = ['id' => $v['user_id'], 'text' => $v['user_full_name']];
            $dd[] = ['id' => $v['user_id'], 'text' => $v['user_full_name'], 'group' => false];
