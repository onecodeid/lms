...
class One_wh_001_a5 extends RPT_Controller
{
    var $report_code;
    ...
    function index() {
        ...
        if ($r)
        {
            ...
            // $this->pdf->Cell(13.3, 0.7, 'No Order : ' . $r['order_number'], '', 0, 'R', 0);
            // $this->pdf->Ln(0.5);
            // $this->pdf->SetFont('Arial','', 10);
            // $this->pdf->Cell(13.3, 0.7, 'Tanggal Invoice : ' . date('d-m-Y', strtotime($r['invoice_date'])), '', 0, 'R', 0);
            // $this->pdf->Ln(0.5);
            ...
            $this->pdf->SetFont('Arial','B', 10);
            $this->pdf->Cell(6, 0.7, 'No Order : ' . $r['order_number'], 'LTR', 0, 'L', 1);
            $this->pdf->Ln(0.5);
            $this->pdf->SetFont('Arial','', 10);
            $this->pdf->Cell(6, 0.7, 'Tanggal Invoice : ' . date('d-m-Y', strtotime($r['invoice_date'])), 'LBR', 0, 'L', 1);

            $this->pdf->SetY($hy + 0);
            $this->pdf->Cell(7, 0.7, '', '', 0, 'L', 0);
            $this->pdf->SetFont('Arial','B', 10);
            ...
            $this->pdf->Ln(0.6);
            $this->pdf->Cell(7, 0.7, '', '', 0, 'L', 0);
            $this->pdf->MultiCell(6.5, 0.5, $r['sh_customer_address'], '', 'L', 0);
            $this->pdf->Cell(7, 0.7, '', '', 0, 'L', 0);
            $this->pdf->Cell(9, 0.7, 'Kec. ' . $r['sh_customer_district'] . ', ' . $r['sh_customer_city'], '', 0, 'L', 0);
            $this->pdf->Ln(0.5);
            $this->pdf->Cell(7, 0.7, '', '', 0, 'L', 0);
            $this->pdf->Cell(9, 0.7, '' . $r['sh_customer_province'] . '  -  ' . $r['sh_customer_postcode'], '', 0, 'L', 0);
            $this->pdf->Ln(0.5);
            $this->pdf->Cell(7, 0.7, '', '', 0, 'L', 0);
            $this->pdf->Cell(9, 0.7, '' . 'HP : ' . $r['sh_customer_phone'], '', 0, 'L', 0);
            ...
            if ($r['expedition_code'] == 'EX.OTHER') {
                ...
                $this->pdf->Cell(9, 0.7, $r['expedition_name'] . ', ' . $r['service_name'] . ', COD', '', 0, 'L', 0);
                $this->pdf->Ln(0.5);
                $this->pdf->Cell(7, 0.7, '', '', 0, 'L', 0);
                $this->pdf->Cell(9, 0.7, 'Pembayaran COD : Rp '.number_format($r['grand_total']), '', 0, 'L', 0);
            } else {
                $this->pdf->Cell(9, 0.7, $r['expedition_name'] . ', ' . $r['service_name'], '', 0, 'L', 0);
            }

            $this->pdf->Ln(1.2);
            if ($this->pdf->GetY() < 6.2)
                $this->pdf->SetY(6.2);
            ...
            foreach ($data as $k => $v)
            {
                ...
                $this->pdf->Cell(10, 1, '  '.$v['item_name'], 'LB', 0, 'L', 1);
