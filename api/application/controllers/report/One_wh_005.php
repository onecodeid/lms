    ...
    function index() {
        ...
        if (isset($this->sys_input['out']))
        {
            if ($this->sys_input['out'] == 'csv')
            {
                ...
                {
                    $data[] = ['NO'=>($k+1),
                        'TANGGAL_ORDER'=>date('d-m-Y', strtotime($v['trans_date'])),
                        'MASUK'=>($v['item_qty'] > 0 ? $v['item_qty'] : 0),
                        'KELUAR'=>($v['item_qty'] < 0 ? $v['item_qty'] : 0),
                        'SALDO'=>$v['after_qty']];
                }

                $this->csv($data, 'kartu_stock_' . $r[0][0]['item_name'] . '_' .
