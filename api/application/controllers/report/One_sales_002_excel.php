...
class One_sales_002_excel extends RPT_Controller
{
    var $report_code;
    // var $pdf;
    ...
    function index() {
        ...
        $r = $this->r_report->one_sales_002( $this->input->get('uid'), $sdate, $edate );
        //
        $grand_total = 0;
        $sub_total = 0;
        
        $filename = "laporan_detail_penjualan_admin_{$r[0][0]['S_UserUsername']}_".date('d.m.Y', strtotime($this->input->get('sdate')))."_".date('d.m.Y', strtotime($this->input->get('edate'))).".xls";
        ...
        if ($r)
        {
            ...
            $as->setCellValue('E1', 'ADMIN : ' . $r[0][0]['S_UserUsername']);
