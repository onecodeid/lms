...
class One_sales_010_excel extends RPT_Controller
{
    var $report_code;
    // var $pdf;
    ...
    function index() {
        ...
        $r = $this->r_report->one_sales_010( $this->sys_input['uid'], $sdate, $edate, $this->sys_input['levelid'] );
        //
        $grand_total = 0;
        $sub_total = 0;

        $filename = "laporan_detail_penjualan_admin_{$r[0][0]['S_UserUsername']}_level_{$r[1][0]['level_name']}_".date('d.m.Y', strtotime($this->input->get('sdate')))."_".date('d.m.Y', strtotime($this->input->get('edate'))).".xls";
        ...
        if ($r)
        {
            ...
            $objPHPExcel = $objReader->load("./assets/template_sales_002.xls");
            ...
            $as->setCellValue('E1', 'ADMIN : ' . $r[0][0]['S_UserUsername'] . ', JENJANG : ' . $r[1][0]['level_name']);
