...
class One_fin_003 extends RPT_Controller
{
    ...
    function index() {
        ...
        $r = $this->r_report->one_fin_003( $this->input->get('sdate'), $this->input->get('edate'), $this->input->get('exp_id') ? $this->input->get('exp_id') : '0' );
        // print_r($r);
        //
        $grand_total = 0;
        $grand_total_qty = 0;
        ...
        if ($r)
        {
            ...
            foreach ($data as $k => $v)
            {
                ...
                $this->pdf->Cell(3, 0.7, $v['so_date'], 'LB', 0, 'L', 1);
                $this->pdf->Cell(9, 0.7, $v['customer_name'], 'LB', 0, 'L', 1);
            ...
            $this->pdf->SetFillColor(222,222,222);
            $this->pdf->SetTextColor(0,0,0);
            $this->pdf->Cell(16, 0.7, 'TOTAL', 'LB', 0, 'C', 1);
            // $this->pdf->Cell(3, 0.7, number_format($total_trx), 'LBR', 0, 'R', 1);
            // $this->pdf->Cell(3, 0.7, number_format($total_qty), 'LBR', 0, 'R', 1);
            $this->pdf->Cell(3, 0.7, number_format($total), 'LBR', 0, 'R', 1);
            $this->pdf->Ln(1);
    ...
    function tableHeader($me)
    {
        ...
        $me->Cell(3, 1, 'TANGGAL' , 'LTBR', 0, 'C', 1);
        $me->Cell(9, 1, 'NAMA CUSTOMER' , 'LTBR', 0, 'C', 1);
