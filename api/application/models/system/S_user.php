...
class S_user extends MY_Model
{
    ...
    function search( $d )
    {
        ...
        $r = $this->db->query(
                ...
                ORDER BY S_UserProfileFullName", [$d['gid'], $d['gid'], $d['gid'], $d['search'], $d['search'], $d['uid'], $d['uid'], $d['uid']]);
        if ($r)
        {
            $r = $r->result_array();
            
            $this->load->model('master/m_fee2');
            foreach ($r as $k => $v)
            {
                $x = $this->m_fee2->search_by_user(['user_id'=>$v['user_id']]);
                $r[$k]['fees'] = $x['records'];
            }
    ...
    function save($d)
    {
        ...
        $jdata = json_encode($d);
        $r = $this->db->query("CALL sp_system_user_save(?, ?, ?, ?)", [$d['user_id'], $d['user_name'], $jdata, $d['fees']])
                        ->row();
    ...
    function username_change($d, $admin_uid, $admin_pass)
    {
        $password = md5($this->pass_prefix . $admin_pass . $this->pass_suffix);
        $r = $this->db->query("CALL sp_system_username_change(?,?,?,?)", [$d['user_id'], $d['user_name'], $admin_uid, $password])
                ->row();
        $this->clean_mysqli_connection($this->db->conn_id);
