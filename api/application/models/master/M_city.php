...
class M_city extends MY_Model
{
    ...
    function search_city_reverse( $d )
    {
        $l = ['records'=>[], 'total'=>0];
        $d['limit'] = 25;
        ...
        $r = $this->db->query(
                "SELECT *
                FROM `v_cities`
                WHERE full_city_reverse LIKE ?
                GROUP BY full_city_reverse
                LIMIT ?", [$d['search'], $d['limit']]);
        if ($r)
        {
            $r = $r->result_array();
        ...
        $r = $this->db->query(
            "SELECT count(DISTINCT city_id) n
                FROM `v_cities`
                WHERE full_city_reverse LIKE ?", [$d['search']]);
        if ($r)
        {
            $l['total'] = $r->row()->n;
        }
