...
class R_stat extends MY_Model
{
    ...
    function Stat_sales_002($uid, $sdate, $edate, $type, $level, $min = 0, $max = 0)
    {
        $r = $this->GetMultipleQueryResult("CALL `sp_stat_sales_002`('{$uid}', '{$sdate}', '{$edate}', '{$type}', '{$level}', '{$min}', '{$max}')", 2);
...
// echo "CALL `sp_stat_sales_005`('{$uid}', '{$sdate}', '{$edate}', '{$item_id}')";
    ...
    // Statistik Omzet Admin per Customer
    function Stat_sales_007($uid, $orderby, $page = 1)
    {
        $r = $this->db->query("CALL `sp_stat_sales_007`('{$uid}', '{$orderby}', '{$page}')")
                    ->result_array();
    ...
    // Analitik Omzet per Bulan
    function Stat_sales_009($month)
    {
        $sdate = $month . "-01";
        $edate = date("Y-m-t", strtotime($month."-01"));

        $r = $this->GetMultipleQueryResult("CALL `sp_stat_sales_009`('{$sdate}', '{$edate}')", 2);
        return $r;
    }
}
